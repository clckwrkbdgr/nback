	...
	: sessionId(0)
{
	...
	QSqlQuery query(db);
	ok = query.exec("CREATE TABLE IF NOT EXISTS nbackSessions (id INTEGER PRIMARY KEY AUTOINCREMENT, startDate TEXT, stopDate TEXT DEFAULT '');");
	if(!ok) showError(query.lastError());
	ok = query.exec("CREATE TABLE IF NOT EXISTS nback (id INTEGER PRIMARY KEY AUTOINCREMENT, sessionId INT, n INT, size INT, errors INT, missed INT);");
	if(!ok) showError(query.lastError());
...
double Statistics::currentErrorRatio() const
{
	QSqlDatabase db = QSqlDatabase::database("stat");
	QSqlQuery query(db);
	query.prepare("SELECT SUM(errors), SUM(missed), SUM(size) FROM nback WHERE sessionId = :sessionId;");
	query.bindValue(":sessionId", sessionId);
	bool ok = query.exec();
	if(!ok) {
		showError(query.lastError());
	...
	query.next();
	double errors = query.value(0).toInt();
	double missed = query.value(1).toInt();
	double total = query.value(2).toInt();
	return (missed + errors) / total;
}
